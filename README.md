...
このプロジェクトは、**Render.com** などのPaaSを利用して簡単にインターネット上に公開（デプロイ）できます。`render.yaml` と `Dockerfile` があらかじめ用意されています。

### 手順

1. このプロジェクトを **GitHub** のリポジトリとしてプッシュします。
2. [Render.com](https://render.com/) にアクセスし、GitHubアカウントでログインします。
3. ダッシュボードから **[New]** -> **[Blueprint]** を選択します。
4. プッシュしたGitHubリポジトリを選択します。
5. `render.yaml` が自動的に読み込まれ、Webサービスとして設定されます。
6. 設定画面の「Environment Variables（環境変数）」セクションで、ご自身の `GEMINI_API_KEY` の値を入力します。（`TUTOR_API_KEY`は自動生成されます）
7. **[Apply]** をクリックすると、自動的にビルドとデプロイが開始されます！
8. ビルドが完了すると、Render.comから `https://ai-tutor-web.onrender.com` のような公開URLが発行され、ブラウザからアクセス可能になります。
